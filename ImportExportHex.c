...
/*
This function reads in Intel 32-bit .hex file formatted firmware image files
and stores the parsed data into buffers in the PC system RAM, so that it is
in a format more suitable for directly programming into the target
microcontroller.
*/
THexImportErrorCode ImportHexFile(THexImporter * HexImporter,
    char * Filename, TDeviceData * PData, TDevice * PDevice)
{
    ...
    unsigned char type;
    THEX32_RECORD recordType;
    unsigned int wordByte;
    THexLine line;

    THexFileIO hexFile; // QFile hexfile(fileName);

    HexImporter->hasConfigBits = false;
    HexImporter->hasEndOfFileRecord = false;
    HexImporter->fileExceedsFlash = false;

    // Open the user specified .hex file.
    if (!HexFile_Open(&hexFile, Filename))
    {
        return hecCouldNotOpenFile;
    }

    // Parse the entire hex file, line by line
    while (!HexFile_atEnd(&hexFile)) //(!hexfile.atEnd())
    {
        unsigned int hexLineChecksum = 0;
        
        // Fetch a line of ASCII text from the .hex file.
        HexFile_ReadNextLine(&hexFile, &line);
        ...
        if ((line.Length < 11) || (line.Chars[0] != ':'))
        {
            ...
            HexFile_close(&hexFile);
            return hecErrorInHexFile;
        }
        ...
        byteCount = AsciiHexToInt(line.Chars, 1, 2);

        lineAddress = segmentAddress + AsciiHexToInt(line.Chars, 3, 4);
        recordType = (THEX32_RECORD)AsciiHexToInt(line.Chars, 7, 2);
        ...
        for(i = 0; i < (byteCount+4); i++)  //+4 correction is for byte count, 16-bit address, and record type bytes
        {
            ...
            wordByte = AsciiHexToInt(line.Chars, 1 + (2 * i), 2);   //Re-format the above two ASCII bytes into a single binary encoded byte (0x00-0xFF)
        ...
        wordByte = AsciiHexToInt(line.Chars, 1 + (2 * i), 2); // hexByte.toInt(&ok, 16);      //Re-format the above two ASCII bytes into a single binary encoded byte (0x00-0xFF)
        wordByte &= 0xFF;
        ...
        {
            HexImporter->hasEndOfFileRecord = true;
            break;
        }
        else if ((recordType == EXTENDED_SEGMENT_ADDR) || (recordType == EXTENDED_LINEAR_ADDR)) // Segment address
        {
            ...
            if ((unsigned int)line.Length >= (11 + (2 * byteCount)))
            {
                ...
                lineAddress = segmentAddress + AsciiHexToInt(line.Chars, 3, 4); //line.mid(3, 4).toInt(&ok, 16);
            }
            else
            {
        ...
        else if (recordType == DATA)   // Data Record
        {
            ...
            for (i = 0; i < byteCount; i++)
            {
                ...
                deviceAddress = Device_GetDeviceAddressFromHexAddress(PDevice,
                    lineAddress + i, PData,
                    &type, &includedInProgrammableRange,
                    &addressWasEndofRange, &bytesPerAddressAndType,
                    &endDeviceAddressofRegion, &pPCRAMBuffer);
                ...
                if ((includedInProgrammableRange == true) && (pPCRAMBuffer != 0)) //Make sure pPCRAMBuffer pointer is valid before using it.
                {
                    ...
                    wordByte = AsciiHexToInt(line.Chars, 9 + (2 * i), 2); // hexByte.toInt(&ok, 16);   //Re-format the above two ASCII bytes into a single binary encoded byte (0x00-0xFF)
                    ...
                    *pPCRAMBuffer = (unsigned char)wordByte;    //Save the .hex file data byte into the PC RAM buffer that holds the data to be programmed
                    ...
                    {
                        HexImporter->hasConfigBits = true;
                    }
                }
